...
export default function RSVPSection() {
  ...
  if (!guest) {
    return (
      <section id="rsvp" className="py-24 px-6 text-center">
        <h2 className={`${antic.className}text-4xl font-bold`}>RSVP</h2>
        <p className={`${antic.className}mt-4 text-gray-600`}>Invitación no encontrada.</p>
      </section>
    );
  }
    ...
    <section id="rsvp" className={`${bodoni.className} py-24 px-6 flex bg-[#FFFAEE] justify-center`}>
      <div className="w-full max-w-xl space-y-6">
        <div className="text-center space-y-3">
          <p>
            <h2 className={`${antic.className} text-5xl text-[#222222] font-bold`}>RSVP</h2>
          </p>
        ...
        {submitted ? (
          <div className="bg-[#FFFDF8] border rounded-2xl p-8 shadow-sm text-center">
            <h2 className={`${antic.className} ext-3xl font-bold`}>¡Gracias, {guest.name}! 🎉</h2>
            <p className={`${antic.className} mt-4 text-gray-700`}>Hemos recibido tu RSVP.</p>
          </div>
        ) : (
          <form
